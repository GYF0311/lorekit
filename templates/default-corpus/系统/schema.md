...
项目本地 evidence 目录（例如 `_工作台/课程原文/`、`_工作台/U单元学习资料/`）可以作为当前任务的证明材料，但不自动属于原料层。只有明确 ingest/promote 后，才进入 `原料/` 并参与长期知识链路。
...
- 中小规模（~100 源，~数百页）下优先用文本检索和索引 drill-down
