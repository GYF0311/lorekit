...
var LLMWikiAuditSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Wiki root").setDesc(
      "Path relative to the vault root. If the vault is the wiki, leave this as `.`."
    ).addText(
      (text) => text.setPlaceholder(".").setValue(this.plugin.settings.wikiRoot).onChange(async (value) => {
        this.plugin.settings.wikiRoot = value.trim() || ".";
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Audit directory").setDesc("Path relative to the wiki root where audit files are written.").addText(
      (text) => text.setPlaceholder("audit").setValue(this.plugin.settings.auditDir).onChange(async (value) => {
        this.plugin.settings.auditDir = value.trim() || "audit";
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Author").setDesc("Written into every audit file's `author` field.").addText(
      (text) => text.setPlaceholder("lewis").setValue(this.plugin.settings.author).onChange(async (value) => {
...
// src/feedback-modal.ts
var import_obsidian2 = require("obsidian");
var SEVERITIES = ["info", "suggest", "warn", "error"];
var FeedbackModal = class extends import_obsidian2.Modal {
  constructor(app, previewText) {
    super(app);
    this.severity = "warn";
    this.comment = "";
    this.resolver = null;
  ...
  onOpen() {
    ...
    new import_obsidian2.ButtonComponent(buttons).setButtonText("Cancel").onClick(() => this.finish(null));
    new import_obsidian2.ButtonComponent(buttons).setButtonText("Save feedback").setCta().onClick(() => this.trySave());
  }
  onClose() {
  ...
  trySave() {
    if (!this.comment.trim()) {
      new import_obsidian2.Notice("Comment is empty");
      return;
    }
...
// ../../audit-shared/node_modules/zod/v3/external.js
...
// ../../audit-shared/node_modules/zod/v3/helpers/util.js
var util;
(function(util2) {
  util2.assertEqual = (_) => {
  };
...
// ../../audit-shared/node_modules/zod/v3/ZodError.js
...
// ../../audit-shared/node_modules/zod/v3/locales/en.js
...
// ../../audit-shared/node_modules/zod/v3/errors.js
...
// ../../audit-shared/node_modules/zod/v3/helpers/parseUtil.js
...
// ../../audit-shared/node_modules/zod/v3/helpers/errorUtil.js
var errorUtil;
(function(errorUtil2) {
...
// ../../audit-shared/node_modules/zod/v3/types.js
...
// ../../audit-shared/dist/schema.js
var Severity = external_exports.enum(["info", "suggest", "warn", "error"]);
var AuditSource = external_exports.enum(["obsidian-plugin", "web-viewer", "manual"]);
var AuditStatus = external_exports.enum(["open", "resolved"]);
var AnchorSchema = external_exports.object({
  target_lines: external_exports.tuple([external_exports.number().int().positive(), external_exports.number().int().positive()]),
  anchor_before: external_exports.string(),
  anchor_text: external_exports.string().min(1),
  anchor_after: external_exports.string()
});
var AuditEntrySchema = external_exports.object({
  id: external_exports.string().regex(/^\d{8}-\d{6}-[0-9a-f]{4}$/),
...
// ../../audit-shared/dist/anchor.js
function computeAnchor(fileText, selStart, selEnd, context = CONTEXT_CHARS) {
  if (selStart < 0 || selEnd > fileText.length || selStart >= selEnd) {
    throw new Error(`computeAnchor: invalid range [${selStart}, ${selEnd}) for text of length ${fileText.length}`);
  }
...
function offsetsToLines(text, start, end) {
  let line = 1;
  let lineStart = 1;
  let lineEnd = 1;
  let seenStart = false;
...
// ../../audit-shared/dist/id.js
function makeId(now = /* @__PURE__ */ new Date()) {
...
function slugify(input, maxLen = 30) {
  return input.toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9\s-]+/g, "").trim().replace(/\s+/g, "-").replace(/-+/g, "-").slice(0, maxLen).replace(/^-+|-+$/g, "");
}
function filenameFor(id, slug) {
  if (!slug)
    return `${id}.md`;
...
// ../../audit-shared/node_modules/js-yaml/dist/js-yaml.mjs
...
// ../../audit-shared/dist/serialize.js
var FRONTMATTER_RE = /^---\n([\s\S]*?)\n---\n?([\s\S]*)$/;
function toMarkdown(entry) {
  const { body, ...front } = entry;
  const yml = dump(front, {
    lineWidth: 0,
    noRefs: true,
    sortKeys: false,
    quotingType: '"',
...
// src/writer.ts
async function writeAudit(app, settings, args) {
  const {
    file,
    fileText,
    selStart,
  ...
  const fullPath = (0, import_obsidian3.normalizePath)(joinRel(auditDirPath, filename));
  ...
  try {
    ...
    new import_obsidian3.Notice(`Audit filed: ${filename}`);
    return created;
  } catch (err) {
    new import_obsidian3.Notice(`Failed to write audit file: ${String(err)}`);
    throw err;
  }
}
...
function resolveTargetRelativeToWiki(vaultPath, wikiRoot) {
  if (wikiRoot === "" || wikiRoot === ".")
    return vaultPath;
  const normalized = wikiRoot.replace(/\/+$/, "");
  if (vaultPath === normalized)
    return "";
...
function joinRel(...parts) {
  return parts.filter((p) => p && p !== ".").map((p) => p.replace(/^\/+|\/+$/g, "")).filter((p) => p.length > 0).join("/");
}
...
// src/main.ts
var LLMWikiAuditPlugin = class extends import_obsidian4.Plugin {
  async onload() {
    ...
    this.addCommand({
      ...
      editorCallback: async (editor, ctx) => {
        if (!(ctx instanceof import_obsidian4.MarkdownView)) {
          new import_obsidian4.Notice("Audit feedback only works in a markdown editor view");
          return;
        }
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (folder) {
          ...
          new import_obsidian4.Notice(`Audit folder: ${folder.path}`);
        } else {
          new import_obsidian4.Notice(`Audit folder not found: ${path}`);
        }
      }
    });
  ...
  async handleAddFeedback(editor, view) {
    ...
    if (selEnd <= selStart) {
      new import_obsidian4.Notice("Empty selection");
      return;
    }
    const preview = selection.length > 280 ? selection.slice(0, 280) + "\u2026" : selection;
  ...
  async handleListFeedbackForCurrentFile() {
    const view = this.app.workspace.getActiveViewOfType(import_obsidian4.MarkdownView);
    if (!view || !view.file) {
      new import_obsidian4.Notice("No markdown file open");
      return;
    }
    ...
    if (!folder) {
      new import_obsidian4.Notice(`Audit folder not found: ${auditDir}`);
      return;
    }
    const matches = [];
    const files = this.app.vault.getMarkdownFiles().filter(
      (f) => f.path.startsWith(auditDir + "/") && !f.path.startsWith(auditDir + "/resolved/")
    );
    ...
    if (matches.length === 0) {
      new import_obsidian4.Notice(`No open audits for ${targetRel}`);
    } else {
...
${matches.join("\n")}`,
  ...
  resolveAuditDir() {
    ...
    if (!root || root === ".")
      return (0, import_obsidian4.normalizePath)(dir);
    return (0, import_obsidian4.normalizePath)(`${root.replace(/\/+$/, "")}/${dir.replace(/^\/+/, "")}`);
  }
};
