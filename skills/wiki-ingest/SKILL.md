...
| host | route | fetcher | 产物类型 |
|---|---|---|---|
| `mp.weixin.qq.com` | `rich` | cheerio + iPhone UA，专门解析微信公众号（标题、作者、`ct` 时间戳、图片懒加载） | `article.md` + `images/` + `source_kind: clipping` |
| `gist.github.com` | `gist` | 拉页面解析 raw 链接 + 拉 markdown 内容 + `<relative-time>` 抽日期 | `article.md` + `source_kind: gist` |
| `github.com` / `www.github.com` | `github` | 仓库 URL → HEAD/README.md；`/blob/<ref>/<path>` URL → 对应 raw 文件 | `article.md` + `source_kind: github` |
| 其他网站 | `rich` | 通用 article / main / body 解析 + OpenGraph/JSON-LD 日期 | `article.md` + `images/` + `source_kind: article` |
| `feishu.cn` / `larkoffice.com` | `lark` | **unsupported**：走 lark-cli docs | — |
| `x.com` / `twitter.com` | `x` | **unsupported**：反爬太强，粘贴截图或文本 | — |
| `*.pdf` | `pdf` | **unsupported**：走 pdf skill | — |

stdout 是**单行 JSON**，解析它决定下一步：

| status | 含义 | 下一步 |
|---|---|---|
| `ok` | 抓取成功 | 读 `markdown` 字段指向的 article.md；按需 `Read` `imagesDir/` 下关键图片。fetcher 会在 `.wiki/ingest-state.json` 写入 `status: fetched` 的记录——你接下来每推进一步，都要调用 `lorekit ingest record <url> --step <archive|wiki|backlink|lint>` 更新它。 |
...
| `error` | 抓取失败（如 `ANTIBOT_BLOCKED` / `raw_fetch_failed`） | 按 `reason` 字段判断回退路径：通用反爬 → 用 `web-access` skill；gist/github raw 失败 → `curl` 直抓 raw URL；其他参考 `suggest` 字段 |
| `unsupported` | 站点 lorekit fetch 不直接处理（lark / x / pdf） | 按 `suggest` 字段使用对应工具（如 lark-cli / pdf skill / WebFetch） |
...
// gist / github 路由产物没有 imagesDir，sourceKind 是 "gist" 或 "github"：
{"status":"ok","route":"gist","url":"...","title":"...","author":"karpathy","publishDate":"2026-04-04","sourceKind":"gist","sourceLayer":"L1","slug":"...","dir":"...","markdown":"...","imagesOk":0,"imagesFailed":0}
```
...
5. **原文落地（铁律）**：用 `mv` 把工作台 slug 目录**搬到**永久位置，**不要用 cp**
   - 公众号 → `原料/剪藏/<slug>/`
   - 一般文章 → `原料/文章/<slug>/`
   - gist / GitHub README / 仓库文档 → `原料/文章/<slug>/`（sourceKind 识别为 `gist` / `github` 即可，归档路径仍走 `原料/文章/`）
   - 书籍笔记 → `原料/书籍/<slug>/`
   - 会议纪要 → `原料/会议/<slug>/`
