...
sequenceDiagram
  ...
  Agent->>State: links closure（suggest/fix/stub/backlog/plain）
  ...
  Sync->>Vec: 有 vector 环境时增量 embed；否则跳过，保持 text mode
```

`--step lint` 是 ingest 的完成门。进入完成门前必须做 links closure：明显别名用 `lorekit links fix <label> --to <slug> --file <file>`，应该存在的节点用 `links stub`，暂不建的节点写入 `系统/missing-nodes.md`，一次性提及用 `links plain`。
...
| SourceFinalize | `commands/source.ts`        | 把 fetch/local source 归档到 `原料/`；本地 source 缺标题时自动补 `title`       |
| Fetcher     | `lib/fetcher/`                 | URL → 本地 markdown + 图片；L1 native fetch，L2 playwright fallback（10 文件子模块，v0.4.0 / 批次 21 拆分） |
| Links       | `commands/links.ts`            | wikilink closure：suggest / fix / stub / backlog / plain，支撑 ingest 关账前闭环 |
...
| 缺失节点 backlog        | `templates/default-corpus/系统/missing-nodes.md`    | links closure 的暂存清单；不是知识页，不进图谱 / 向量 / _INDEX                         |

## 外部依赖契约

| 依赖            | 接口                             | 失败降级                                               |
| --------------- | -------------------------------- | ------------------------------------------------------ |
| ollama          | `POST localhost:11434/api/embed` | `sync` 跳过向量并保持 text mode；显式 `vector sync` 再提示用户启动 |
| sqlite-vec      | dynamic import                   | `optionalDependencies`；`sync` 可跳过，显式 vector 命令报错并提示安装 |
