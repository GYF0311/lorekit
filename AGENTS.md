...
lorekit gbrain export --out .wiki/integrations/custom-gbrain-export # 自定义 out 必须留在 integrations 下
...
lorekit gbrain query "<q>"                  # 查询前默认检查 freshness；stale 只 warn，不阻止查询
lorekit doctor --json                       # 机器可读健康报告，含 optional integrations
lorekit doctor --section integrations       # 严格 section 校验；可选值见 --help
```
