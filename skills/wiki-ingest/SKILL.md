...
| 原料归档收尾（`source finalize`）      | 判断来源类型 / 本地标题取值    |
| 链接闭环（`links suggest/fix/...`）    | 判断 stub / backlog / plain    |
...
按 `系统/filing-rules.md` 路由到归档目录。优先用 CLI 做归档收尾：

```bash
lorekit source finalize 原料/文章/<slug>.md
```

对本地文件 / 粘贴文本，先按 filing rules 放入 `原料/`，再用 `source finalize` 补齐 `slug` / `raw_sha256` / `last_verified` / `possibly_outdated`，避免后续 lint 才发现 frontmatter 不完整。

若当前版本没有 `source finalize`，再按下面的 `mv` fallback 执行。
...
### 5.5 Links closure — `lorekit links ...`

对本次新建 / 修改的 wiki 页逐个跑：

```bash
lorekit links suggest --file "知识库/实体/A.md" --json --write-state
```
...
| 情况 | 动作 |
| --- | --- |
| 明显别名 / 路径漂移 / 大小写差异 | `lorekit links fix "<label>" --to "<canonical-slug>" --alias "<label>" --file "<file>"` |
| 这个节点值得存在，但还没内容 | `lorekit links stub "<label>" --type concept|entity --source "<file>"` |
| 以后可能要补，但本次不值得建页 | `lorekit links backlog "<label>" --type concept|entity --source "<file>"` |
| 一次性提及，不应成为 wiki 节点 | `lorekit links plain "<label>" --file "<file>"` |

只有 links closure 完成后，才进入 Step 6 关账。`系统/missing-nodes.md` 里的 backlog 不阻断完成，但必须有 reason。
...
- 本地文件 / 粘贴文本若没有 URL 标题，`ingest record` 要记录本地 source title，保证后续 frontmatter / log 可读
...
- 有 vector 环境时增量向量同步；无 vector 依赖 / ollama 时跳过，保持 text mode
- doctor 健康检查
...
links closure：2 fixed, 1 stub, 1 backlog, 0 P0 unresolved
一次关账：record --step archive,wiki,backlink,lint --log "..."
sync：index.md +2 added, 6 _INDEX.md refreshed, vector skipped in text mode
...
- `lorekit source finalize 原料/文章/<source>.md` — 原料归档收尾，补 slug/hash/verified 字段
- `lorekit ingest check <files...>` — 死链预检（写完页立刻跑）
- `lorekit links suggest/fix/stub/backlog/plain` — links closure（关账前必须完成）
...
- `lorekit sync` — 一条命令：刷 \_INDEX + merge index.md + 向量（可跳过）+ doctor
- `lorekit lint --json` / `lorekit lint plan` / `lorekit lint fix --safe` — 全局健康与安全修复
